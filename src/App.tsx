...
function App() {
  ...
  const nextQuestion = () => { const nextQuestion = number + 1; (nextQuestion === total_Questions) ? setGameOver(true) : setNumber(nextQuestion); }
  ...
  const TypeHandler = (event: React.ChangeEvent<HTMLInputElement>) => { setType((event.target as HTMLInputElement).value); };
      ...
      <Wrapper>
        ...
        {gameOver || userAnswer.length === total_Questions ? (
          <div>
            ...
            <div className="center"><button className="start" onClick={startQuiz}><b>Start</b></button></div>
          </div>
        ) : null}
